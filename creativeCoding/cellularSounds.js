...
function preload() {
  song = loadSound("creativeCoding/JamiexxKILLDEM.mp3");
}
...
function setup() {
  ...
  canvas.parent("cellularSoundsCanvas");
...
function draw() {
  ...
  separationAmplify = map(amplitudeLevels, 0, 0.5, 0, 3); // changes cell separation in relation to amplitude
  cellAmplify = map(amplitudeLevels, 0, 0.5, -5, 15); // changes strokeweight in relation to amplitude
...
// in the future i want to look into quad trees for efficiancy
